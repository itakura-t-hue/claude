...
記事メタ情報+KPIカード / パフォーマンス推移グラフ（SS推移・順位推移） / 企画案プレビュー / 初稿プレビュー（すべてGoogleドキュメントで作成。Google Docs API経由でプレビュー表示） / フィードバック・指摘スレッド / 進行タイムライン
...
| オーガニックSS/CV | `sync-ga4`バッチ完了時に更新 | TrafficSnapshot（既存） |
...
- 不適切な出力時: テンプレートベースのフォールバック所感を表示（「前月比でオーガニックSSが{diff}%{増加/減少}しました。」等）
