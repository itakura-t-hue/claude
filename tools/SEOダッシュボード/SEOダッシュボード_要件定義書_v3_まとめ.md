...
| コンポーネント | 表示内容 | データソース |
|-------------|---------|------------|
| **アップデートタイムライン** | Google公式アップデート（手動登録）+ シークレットアップデート（自動検知）を時系列表示 | 手動登録 + 自動検知 |
| **影響度ヒートマップ** | アップデート前後で全案件の順位・流入変動を色分け表示 | RankHistory, TrafficSnapshot |
| **週次トレンドチャート** | 全案件の順位変動を集約した「業界平均」推移線 | RankHistory全案件集計 |
| **アラート履歴** | シークレットアップデート検知アラートの一覧 | AlertHistory |

> **💼 ビジネス解説：シークレットアップデート検知とは**
> Googleは公式発表なしにアルゴリズムを変更することがあります（シークレットアップデート）。全案件の順位データを横断的に分析し、同時期に多数のクライアントで共通の順位変動が発生した場合、アルゴリズム変動の可能性としてアラートを出します。これにより「なぜ急に順位が下がったのか」への初動対応が早まります。

**シークレットアップデート検知ロジック:**

```
検知条件:
  - 直近3日間で、全トラッキングKWの20%以上が±3位以上変動
  - かつ、変動が10社以上のクライアントで同時に発生
  - かつ、変動方向（上昇/下落）に偏りがある（70%以上が同方向）

→ AlertHistory に severity='critical', type='algorithm_update_suspected' で記録
→ Slack即時通知 + 管理者メール
```
