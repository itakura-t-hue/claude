...
- **初年度ROI**: (30,960,000 - 2,230,000 - 11,200,000) / 11,200,000 = **156%**
- **投資回収期間: 4-5ヶ月**
- 2年目以降は年間¥28,700,000以上の純効果
...
**コスト試算の検証が必要:**
- 1社あたりの入力トークン数（推定 2,000-4,000トークン）
- 1社あたりの出力トークン数（推定 3,000-5,000トークン）
- 120社 × 月1回 = 月120回。claude-sonnet-4-6の料金で月額を再計算

**失敗時のフォールバック:**
- Claude API障害時: 所感欄を「AI所感の生成に失敗しました。手動で入力してください。」として空欄で先に進む
