...
| 6 | **BPOへの手作業委託コスト** | ベトナム事業所にLooker Studioスクショ→PPTX貼付作業を月5万円で委託。手作業のため品質にばらつき | レポート自動生成で完全自動化。BPOコスト月5万円を削減 |
...
| Week | マイルストーン | 成果物 | 工数(人日) |
|------|-------------|--------|-----------|
| W23-24 | **M11: レポート自動生成** | PPTXテンプレート、データバインディング、生成パイプライン | 12 |
...
#### P2-1. PPTX内グラフ描画の技術選定

| 選択肢 | メリット | デメリット |
|--------|---------|----------|
| **(A) pptxgenjsネイティブチャート** | 追加依存なし、軽量 | グラフの見た目がRechartsと異なる、カスタマイズ性が低い |
| **(B) Puppeteer + Rechartsスクリーンショット** | ダッシュボードと同じ見た目 | Cloud Runでヘッドレスブラウザ運用のオーバーヘッド、生成速度が遅い |
| **(C) Chart.js + canvas → PNG → PPTX埋め込み** | サーバーサイドCanvas描画、中間の選択肢 | Chart.jsの追加学習コスト |

**推奨:** Phase 3開始前に、現行レポートの1スライドを各方式でPoC実施し、品質・速度・工数を比較。
