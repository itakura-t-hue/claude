...
> 現在のチーム体制は **プリセールス3-4名、ディレクター6名、コンテンツ担当5名、エンジニア3名、セールス4-6名（+新卒4名入社予定）** です。1案件にプリセールス1名+ディレクター1名が基本アサインされ、総計120〜180案件を運用しています。案件数に対して人員が足りず、対応品質が落ちることで解約が発生しています。
...
| 項目 | 現状 |
|------|------|
| **チーム規模** | プリセールス3-4名、ディレクター6名、コンテンツ担当5名、エンジニア3名、セールス4-6名（+新卒4名入社予定） |
| **1案件あたりのアサイン** | プリセールス1名 + ディレクター1名（必須）。コンテンツ制作案件はコンテンツ担当1名追加。実装案件はエンジニアをアサイン。セールスは新規明けテク後は基本担当から外れるが定例会に参加することがある |
| **総案件数** | 約120〜180案件 |
...
| 6 | **BPOへの手作業委託コスト** | ベトナム事業所にLooker Studioスクショ→PPTX貼付作業を月5万円で委託。手作業のため品質にばらつき | Looker Studio埋め込み + AI所感 + PDF出力で完全自動化。BPOコスト月5万円を削減 |
...
| 優先度 | 課題 | 解決策 | 期待効果 |
|--------|------|--------|----------|
| **1位** | **コンテンツ進捗管理の一元化（炎上→解約防止）** | コンテンツ管理 + クライアントポータル（Phase 1）、チャット（Phase 2） | 大量記事発注の炎上→解約を防止 |
...
|---------|---------|---------------------|------------------|
| **Phase 1 解約阻止** | 3ヶ月 | コンテンツ進捗管理 + クライアントポータル + ライターポータル | 炎上→解約の防止。クライアントが「自分で確認できる」状態を実現 |
| **Phase 2 スケール** | 2ヶ月 | GSC/GA4/Ahrefs/CMS連携 + 概要ダッシュボード（120案件スケール） + チャット + 通知設計 | データ収集作業ゼロ。120案件を1画面で俯瞰管理。チャットによる連絡一元化 |
| **Phase 3 ロックイン** | 2ヶ月 | レポート自動生成 + AI改善提案 + 保守プラン設計 + ナレッジベース | レポート工数削減 + 保守プランによる解約阻止ロックイン |
| **Phase 4 高度分析** | 1ヶ月 | Site Audit深度統合 + 高度分析ダッシュボード | BigQueryに蓄積済みデータを活用した高度分析機能。※BigQueryへのデータ蓄積自体はPhase 2で開始 |
...
> - **GA4の管理者権限への昇格依頼** — 各案件で使用しているGmailアカウントの権限を「閲覧者」→「管理者」に変更してもらう（BigQueryエクスポート設定を自社で代行するため）
...
> - **保守プランのロックイン**: GA4ローデータ含む全データが自社管理下にあるため、解約時にはこの蓄積データへのアクセスが全て失われる。特にGA4ローデータはGA4本体ではデフォルト2ヶ月（最大設定でも14ヶ月）で消えるが、自社BQなら永久保持

> **🎯 GA4ローデータ蓄積のメリット（コンサルの提案幅が大幅拡大）**
> GA4 Data API（集計データ）だけでは「2月のセッションは40,600」止まりです。BigQueryにローデータを蓄積すると：
...
|---------|------|--------------|------------|-------------------|
| **GA4（ローデータ）** | 案件ごとのGmailアカウントに**管理者権限**（現行「閲覧者」から昇格依頼） | GA4管理画面のBigQueryエクスポート機能を**自社が設定代行**。エクスポート先を自社GCPのBigQueryデータセットに指定。毎日自動送信 | 全イベントの生データ（page_view, scroll, click, purchase等） | GA4権限を「閲覧者→管理者」に変更（1回のみ） |
| **GA4（集計データ）** | 同上（案件ごとのGmailアカウント、管理者権限に包含） | GA4 Data API（OAuth 2.0）で日次バッチ取得 → 自社BQにINSERT | セッション数、ユーザー数、CV数、チャンネル別、LP別等 | なし |
| **GSC** | 案件ごとのGmailアカウントに閲覧者権限（現行通り） | Search Console API（OAuth 2.0）で日次バッチ取得 → 自社BQにINSERT | クエリ別クリック/表示/CTR/順位、ページ別データ | なし |
...
| **認証** | OAuth 2.0。クライアントごとのGmailアカウント（案件により異なる）でリフレッシュトークンをPostgreSQLに暗号化保存 |
...
| **ページ別パフォーマンス** | `dimensions: ['page']` | ランディングページTOP20 | 全ページ取得、セッション数降順ソート |
...
| **認証** | OAuth 2.0。クライアントごとのGmailアカウント（案件により異なる。GA4管理者権限に昇格依頼） |
...
| 項目 | 仕様 |
|------|------|
| **概要** | 自社運営の順位自動取得ツール。Yahooをスクレイピングし、登録ドメインの対象KWにおける日次掲載順位を取得 |
| **データ取得** | haveのDBから日次順位データを取得（API or DB直接参照） |
| **フロント** | ブラウザベースのWebアプリとして既に稼働中 |
| **KW登録運用** | 案件開始時にクライアントとKWを擦り合わせて登録。以後、随時追加 |
| **GSCとの使い分け** | GSC = Google検索の実績データ（クリック・表示回数・CTR含む）。have = Yahoo検索の順位データ（日次）。両方を併用し、Google/Yahoo両方の順位を把握 |
| **同期頻度** | 日次（sync-gscと同タイミング、03:00 JST） |

> **💼 ビジネス解説：なぜGSCとhaveの両方が必要か**
> GSCはGoogleの検索データしか取れません。日本ではYahoo!検索のシェアも一定あるため、haveでYahoo順位を取得しています。また、GSCは3日遅れのデータですが、haveは当日の順位をリアルタイムに近い形で確認できます。
...
| **GA4ローデータ** | GA4本体はデフォルト2ヶ月（最大14ヶ月）で消える | 保守プロ契約中は蓄積継続。解約すると空白期間が発生 |
...
model RankHistory {
  ...
  ctr           Float?
  url           String?
  source        String    @default("gsc")  // "gsc" | "have"
...
| **オーガニックセッション** | 当月値 + 前月比% | `TrafficSnapshot` |
...
function calculateHealthScore(data: ProjectMetrics): number {
  const scores = {
    ...
    ranking: (data.kwInTop10 / data.totalTrackedKw) * 25,                     // 順位 (0-25)
...
| コンポーネント | 表示内容 | データソース |
|-------------|---------|------------|
| **指標切替タブ × 4** | (1) オーガニックセッション (2) ユーザー数 (3) CV数 (4) 被リンク数 | GA4 / Ahrefs |
...
**カード内表示:** 記事タイトル / 対象KW+検索Vol / 担当ライター / 〆切日+残日数 / 検索順位 / 月間セッション / セッション推移スパークライン / 順位推移スパークライン / ミニKPIカード
...
| 経年劣化 | 公開6ヶ月以上 & 過去3ヶ月のセッションが減少傾向 | `aged` |
...
| **原稿提出** | Google Docs URL登録（執筆はすべてGoogleドキュメントで実施） | 提出 |
...
記事メタ情報+KPIカード / パフォーマンス推移グラフ（セッション推移・順位推移） / 企画案プレビュー / 初稿プレビュー（すべてGoogleドキュメントで作成。Google Docs API経由でプレビュー表示） / フィードバック・指摘スレッド / 進行タイムライン
...
Looker Studio埋め込み（左） + AI所感編集エリア（右）（全17セクション） / PDF出力（@media print + window.print()）
...
### 画面 12: クライアントポータル (`/portal/[id]`) — Phase 1

ウェルカムヘッダー / KPIカード / セッション推移グラフ / 承認待ちコンテンツ一覧 / 月次レポートアーカイブ

### 画面 13: チャット (`/clients/[id]/chat`) — Phase 2
...
### 画面 14: アップデート分析 (`/updates`) — Phase 2

**対応する業務課題:** Googleアルゴリズムアップデート（公式・シークレット）の影響を迅速に把握し、クライアントへの初動対応を早める
...
│  データ収集      │ ──▶ │  横断分析エンジン     │ ──▶ │  検知・通知      │
│                 │     │                     │     │  ・レポート      │
│ GSC API（日次）  │     │ 120社の順位・流入     │     │                 │
│ have（Yahoo日次）│     │ データを同時分析      │     │ Slack即時通知    │
│ GA4 API（日次）  │     │                     │     │ メールアラート    │
│ Ahrefs API      │     │ 3条件で変動判定:      │     │ 週次トレンド     │
│                 │     │ ①変動率 ②影響範囲    │     │ 影響度ヒートマップ │
│ 毎日03:00-04:30 │     │ ③方向性の偏り        │     │ 1次情報レポート   │
│ JST 自動実行    │     │                     │     │                 │
└─────────────────┘     └─────────────────────┘     └─────────────────┘
```
...
| データソース | 取得データ | 頻度 |
|------------|----------|------|
| **GSC API** | Google検索の順位・クリック・表示回数（全トラッキングKW） | 日次（03:00 JST） |
| **haveツール** | Yahoo検索の日次順位（全登録KW） | 日次（03:00 JST） |
| **GA4 Data API** | オーガニックセッション・ユーザー数・CV数 | 日次（03:30 JST） |
| **Ahrefs API** | DR・被リンク数・参照ドメイン数 | 日次/週次 |

全データはBigQueryに蓄積され、横断分析の入力データとなる。

#### STEP 2: 横断分析エンジン

**シークレットアップデート検知ロジック（3条件AND判定）:**

| # | 条件 | 閾値 | 判定対象 |
|---|------|------|---------|
| 1 | **順位変動率** | 直近3日間で、全トラッキングKWの**20%以上**が±3位以上変動 | 全案件のKWを集約して判定 |
| 2 | **影響範囲** | 変動が**10社以上**のクライアントで同時に発生 | クライアント単位でカウント |
| 3 | **方向性の偏り** | 変動方向（上昇/下落）に**70%以上の偏り**がある | 上昇KW数 vs 下落KW数 |

3条件すべてを満たした場合 → `AlertHistory` に `severity='critical'`, `type='algorithm_update_suspected'` で記録。

**週次トレンド分析（毎週月曜自動実行）:**

| 分析項目 | 内容 |
|---------|------|
| **全案件平均順位変動** | 120社の全KW平均順位の週次推移。急激な変動を可視化 |
| **業界別影響度** | クライアントの業界カテゴリごとにグルーピングし、業界別の影響差を分析 |
| **Google vs Yahoo比較** | GSC（Google）とhave（Yahoo）の順位変動を比較。Google単独のアップデートかどうかを判別 |
| **流入影響の遅延分析** | 順位変動から流入変動までのタイムラグを分析。順位は変わらないがAIOの影響でCTRが低下しているケースを検出 |

#### STEP 3: 検知・通知・レポート

| 出力 | トリガー | 配信先 | 内容 |
|------|---------|--------|------|
| **シークレットアップデートアラート** | 3条件AND判定が成立 | Slack即時 + 管理者メール | 「アルゴリズム変動の可能性を検知。影響を受けたクライアント: ○社。主な影響KWカテゴリ: ○○」 |
| **週次トレンドレポート** | 毎週月曜 08:00 JST | Slack + メール（全コンサルタント） | 全案件の週次サマリー。平均順位変動・流入変動・注目すべきクライアントTOP5 |
| **影響度ヒートマップ** | アップデート検知時に自動生成 | ダッシュボード画面 | クライアント×KWカテゴリのマトリクス。赤=悪影響 / 緑=好影響 / グレー=変動なし |
| **1次情報レポート** | 手動トリガー（コンサルタント操作） | 外部発信用（ブログ・SNS・セミナー） | 120社の匿名集計データに基づく独自分析レポート。ブランディング・リード獲得に活用 |

#### 画面UI構成

| コンポーネント | 表示内容 | データソース |
|-------------|---------|------------|
| **アップデートタイムライン** | Google公式アップデート（手動登録）+ シークレットアップデート（自動検知）を時系列表示。公式は青バッジ、シークレットは赤バッジで区別 | 手動登録 + AlertHistory |
| **影響度ヒートマップ** | アップデート前後で全案件の順位・流入変動を色分け表示。行=クライアント、列=KWカテゴリ | RankHistory, TrafficSnapshot |
| **週次トレンドチャート** | 全案件の順位変動を集約した「業界平均」推移線。Google/Yahoo別の2本線 | RankHistory（GSC + have）全案件集計 |
| **Google vs Yahooタブ** | 同一KWのGoogle順位（GSC）とYahoo順位（have）の乖離を表示。Google単独アップデートの判別に使用 | RankHistory（source='gsc' vs source='have'） |
| **アラート履歴** | シークレットアップデート検知アラートの一覧。日時・影響社数・影響KW数・対応ステータス | AlertHistory |

> **💼 ビジネス解説：なぜこの機能が武器になるか**
> 通常のSEOコンサル会社は、アルゴリズムアップデートを「SEO系メディアの記事」で知ります。しかし本ツールでは**自社で120社のデータを横断分析し、業界で最も早くアップデートを検知**できます。この速度差がクライアントへの信頼感と「このコンサル会社は他と違う」という差別化に直結します。さらに、匿名集計データに基づく1次情報の発信は、SEO業界でのプレゼンス向上と新規リード獲得に貢献します。
...
| `GET` | `/api/clients/[id]/reports/[rid]/download` | PDF出力URL取得 | アサイン済み | P2 |
| | **アラート** | | | |
...
| **`sync-have`** | 03:00 | haveのDBから日次順位データ取得 → `RankHistory`にマージ（source='have'） | 約10秒 | 3回（5min, 15min, 60min） |
...
| **流入急減** | 日次オーガニックセッションが過去7日平均の70%以下 | Critical | Slack即時 |
...
| **アルゴリズムアップデート検知** | 全案件の順位・流入を横断分析し、共通の大幅変動パターンを検知 | Critical | Slack即時 + メール |
| **週次トレンドレポート** | 毎週月曜に全案件の週次分析を自動実行。トレンド変動サマリーを配信 | Info | Slack + メール（週次ダイジェスト） |
...
1. 生成ボタン押下 → 2. AI所感生成 → 3. Looker Studio + 所感プレビュー → 4. 所感編集 → 5. PDF出力（@media print + window.print()）

### レポートセクション構成（現行レポートとの対応）

| セクション# | 内容 | 現行スライド | データソース（Looker Studio） | AI所感 |
|------------|------|------------|---------------------------|--------|
| 1 | 表紙 | 表紙 | Client情報 | - |
...
| 6 | オーガニックセッション推移 | スライド10 | GA4 organicSessions | 自動生成 |
...
> **💼 ビジネス解説：なぜ「1次情報」が重要か**
> SEO業界では多くの情報が二次的な引用や推測に基づいています。本ツールで蓄積した120+社のデータを匿名化・集計することで、「自社独自の分析に基づく1次情報」を発信できます。これはSEOコンサルティング会社としてのブランディングと、新規リード獲得に直結します。

**活用例:**

| 1次情報 | データソース | 発信形態 |
|---------|------------|---------|
| **業界別SEOベンチマーク** | 120社の匿名集計データ（DR平均、順位分布、CVR等） | ホワイトペーパー、ブログ記事 |
| **アルゴリズムアップデート影響レポート** | 全案件の横断分析結果 | 速報ブログ、SNS投稿 |
| **コンテンツ施策の効果統計** | KnowledgeBaseの施策→効果データを集計 | セミナー資料、営業提案資料 |
| **Yahoo vs Google順位乖離レポート** | have（Yahoo）とGSC（Google）の比較分析 | 業界レポート |

**実装上の留意点:**
- クライアントデータの匿名化は必須（社名・ドメインを除去し、業界カテゴリ単位で集計）
- データの利用許諾をクライアント契約書に含める必要あり（法務確認事項）
...
| **ファイルストレージ** | Cloud Storage (GCS) | - | レポートPDF保存 |
| **AI（所感生成）** | Claude API or Gemini（検討中） | - | 所感ドラフト・改善提案生成。下記「AI基盤の選定」参照 |
| **レポート可視化** | Looker Studio | - | BQ連携レポート可視化。iframeでダッシュボードに埋め込み |
...
| 機能 | 内容 | フェーズ |
|------|------|---------|
| **GA4/GSC/Ahrefs → 自社BigQuery蓄積** | 各APIで取得したデータを自社バッチで自社BQにINSERT | **P2**（データ連携と同時に開始） |
...
| W7-8 | **M4: ライターポータル** | ライター用UI、原稿提出、フィードバック確認、評価システム | 10 |
| W9-10 | **M5: テスト・改善** | E2Eテスト、承認フロー検証、自動リマインダー、バグ修正 | 10 |
| | | **Phase 1 小計** | **56人日** |

### Phase 2: スケール対応（W11-22 = 約3ヶ月）

| Week | マイルストーン | 成果物 | 工数(人日) |
|------|-------------|--------|-----------|
| W11-12 | **M6: GSC/GA4連携** | OAuth認証フロー、APIクライアント、日次バッチジョブ | 12 |
| W13-14 | **M7: Ahrefs/CMS連携** | Ahrefs APIクライアント、WP REST API、Webhook | 10 |
| W15-16 | **M8: ダッシュボード** | 全案件テーブルビュー、クライアント概要、KW管理、外部リンク | 14 |
| W17-18 | **M9: チャット機能** | チャンネル管理、WebSocket、メンション・参照、ファイル添付 | 14 |
| W19-20 | **M10: 通知設計** | アラートルールエンジン、優先度制御、日次ダイジェスト | 10 |
| | | **Phase 2 小計** | **60人日** |

### Phase 3: ロックイン（W23-30 = 約2ヶ月）

| Week | マイルストーン | 成果物 | 工数(人日) |
|------|-------------|--------|-----------|
| W23-24 | **M11: レポート自動生成** | Looker Studioテンプレート作成、iframe埋め込み、所感編集UI、PDF出力 | 12 |
| W25-26 | **M12: AI所感 + 改善提案** | Claude API連携、所感ドラフト、月2案自動生成、リライトスコアリング | 12 |
| W27-28 | **M13: 保守プラン + ヘルススコア** | 保守プランUI、解約時比較画面、ヘルススコア算出 | 10 |
| W29-30 | **M14: ナレッジベース** | 施策→実施→効果の記録UI、検索・フィルタ、AI連携 | 10 |
| | | **Phase 3 小計** | **44人日** |

### Phase 4: データ分析基盤強化（W31-34 = 約1ヶ月）

| Week | マイルストーン | 成果物 | 工数(人日) |
|------|-------------|--------|-----------|
| W31-32 | **M15: Site Audit + BigQuery高度分析** | Site Audit深度統合、高度分析ダッシュボード | 10 |
| W33-34 | **M16: 高度分析ダッシュボード + リリース** | 総合スコア、Tool ROI、競合ベンチマーク、全体テスト | 10 |
| | | **Phase 4 小計** | **20人日** |
...
| AI API（所感・提案生成） | ¥1,000-5,000 | モデル選定中（Gemini Flash〜Claude Sonnet）。120社×月1レポート+月2提案 |
| Pusher（リアルタイムチャット） | ¥8,000 | Startup plan $49/月 |
| Cloud Storage | ¥1,000 | レポートPDF保存 |
| Looker Studio | ¥0 | Googleアカウントがあれば無料。BQ連携レポート埋め込みに使用 |
| Sentry + 監視 | ¥5,000 | エラー監視 + 死活監視 |
| **インフラ月額合計** | **¥125,000-156,000** | |
| **インフラ年額** | **¥1,500,000-1,872,000** | |
...
| **保守プラン転換** | **¥3,720,000** | 解約予定10件のうち5件が保守プランに移行（ライト3件×¥50,000 + プロ2件×¥80,000 = ¥310,000/月）× 12ヶ月 |
| **レポート工数削減** | **¥8,640,000** | 120社 × 月2時間削減 × ¥3,000/時 × 12ヶ月 |
| **担当可能案件数増加** | **¥18,000,000** | 1人あたり+5件 × 10人 × 月額¥300,000 × 12ヶ月（ポテンシャル） |
| **BPOレポート作業削減** | **¥600,000** | ベトナムBPOへの月額¥50,000 × 12ヶ月。レポート自動生成で完全代替 |
| **年間効果合計** | **¥30,960,000〜** | |

### ROIまとめ

- **開発投資**: ¥9,200,000-11,200,000（一時費用）
- **年間インフラコスト**: ¥1,500,000-1,872,000
- **年間効果（解約防止+保守転換+工数削減）**: ¥30,960,000
- **初年度ROI**: (30,960,000 - 1,872,000 - 11,200,000) / 11,200,000 = **159%**
- **投資回収期間: 4-5ヶ月**
- 2年目以降は年間¥29,000,000以上の純効果

---

## 19. エンジニア確認事項・技術仕様補足

> **⚠️ 本セクションは開発着手前にエンジニアチームでレビュー・決定が必要な事項をまとめたものです。**
> 優先度は **P0（着手前に必須）/ P1（Phase 2までに必須）/ P2（Phase 3までに必須）/ P3（全Phase横断）** で分類。

### P0: Phase 1着手前に決定必須

#### P0-1. OAuthフローの設計（2種類の認証の分離）

本システムには **2つの異なるOAuth用途** が存在する。仕様上「OAuth 2.0」と一括りにされているが、実装は全く別物。

| 用途 | 目的 | アカウント | スコープ |
|------|------|-----------|---------|
| **ユーザーログイン** | ダッシュボードへのログイン認証 | 各ユーザーの個人Googleアカウント | `openid`, `email`, `profile` |
| **データ取得** | GSC/GA4 APIからのデータ同期 | クライアントごとのGmailアカウント（案件により異なる。クライアントから権限付与済み） | `webmasters.readonly`, `analytics.readonly` |

**決定が必要な事項:**
- **(A)** 共有Gmail 1アカウントで全クライアントのGSC/GA4にアクセスする方式 → トークンはグローバルに1つ。Projectモデルの `googleAccessToken` / `googleRefreshToken` は不要（削除 or 別テーブルへ移動）
- **(B)** クライアントごとに別のGoogleアカウントで認証する方式 → Projectモデルのトークンフィールドをそのまま使用
- **(C)** GCPサービスアカウント + ドメイン全体の委任方式 → OAuthトークン管理が不要になるが、クライアント側の設定負荷が増える

**補足:** 現行運用ではクライアントから権限をいただくGmailアカウントは **案件ごとに異なることがある**。そのため **(B) クライアントごとに個別トークン管理** が実態に即した方式。Projectモデルの `googleAccessToken` / `googleRefreshToken` をそのまま活用し、案件ごとに異なるGmailアカウントのOAuthトークンを個別管理する。トークン失効時は設定画面から案件単位で再認証フローを実行。

#### P0-2. クライアント・ライターの招待・認証フロー

現仕様ではGoogle OAuthのみだが、以下のケースで不足する:

| ユーザー種別 | 課題 | 検討すべき代替手段 |
|------------|------|------------------|
| **クライアント担当者** | Googleアカウントを業務で使っていない企業がある | メール+パスワード認証、マジックリンク（メールOTP） |
| **外部ライター** | クラウドワークス経由の都度公募。Google強制は摩擦が大きい | マジックリンク、招待URLトークン |
| **フリーランス** | 案件終了後のアカウント無効化フローが未定義 | 有効期限付きアクセストークン、プロジェクト単位の有効/無効切替 |

**決定が必要な事項:**
- Google OAuth以外の認証手段を追加するか（NextAuth.jsはEmail Provider対応可）
- ユーザー招待フロー: 管理者が招待メール送信 → 受信者がリンクからアカウント作成、の設計
- ライターのライフサイクル管理: 招待 → アサイン → 案件完了 → アクセス無効化

#### P0-3. API共通仕様

全APIエンドポイントに適用する共通仕様が未定義。Phase 1のAPI実装前に以下を確定する必要がある。

**ページネーション:**

| 方式 | 候補 | 適用先 |
|------|------|--------|
| **Cursor-based** | `?cursor=xxx&limit=50` | チャットメッセージ、監査ログ等の時系列データ |
| **Offset-based** | `?page=1&per_page=50` | クライアント一覧、KW一覧等の一覧系 |

デフォルトページサイズ: 50件、最大ページサイズ: 200件（要決定）
...
// 成功時
{
  "data": T | T[],
  "meta": {
    "total": number,       // 総件数（一覧系のみ）
    "page": number,        // 現在ページ（offset-based）
    "per_page": number,
    "next_cursor": string  // 次カーソル（cursor-based）
  }
}
...
{
  "error": {
    "code": string,        // "UNAUTHORIZED" | "FORBIDDEN" | "NOT_FOUND" | "VALIDATION_ERROR" | "RATE_LIMITED" | "INTERNAL"
    "message": string,     // 人間可読なメッセージ
    "details": object      // バリデーションエラーの詳細等（任意）
  }
}
```
...
| ロール | 制限 | 実装方式 |
|--------|------|---------|
| 社内ユーザー | 600 req/min | Redis Token Bucket |
| クライアント | 120 req/min | 同上 |
| ライター | 60 req/min | 同上 |
| Webhook | 30 req/min（クライアントごと） | 同上 |

レスポンスヘッダー: `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`

#### P0-4. 時系列データの格納先（PostgreSQL vs BigQuery）

本書ではRankHistory, TrafficSnapshot, BacklinkSnapshot, ContentTrafficHistoryについて、以下の2箇所で異なる記述があります。

| 箇所 | 記述 |
|------|------|
| セクション5冒頭 | BigQueryに格納（日付パーティション、追記専用） |
| セクション5 Prisma Schema | PostgreSQLモデルとして定義 |

**決定が必要な選択肢:**
- **(A)** BigQueryのみ — Prisma SchemaからRankHistory等を削除。アプリからはBigQuery APIで取得
- **(B)** PostgreSQLのみ — Phase 1-3はPostgreSQLで運用。BigQueryはPhase 4で分析用の二次ストアとして導入
- **(C)** デュアルライト — PostgreSQLをリアルタイム表示用、BigQueryを長期分析用。バッチ処理で両方にINSERT

#### P0-5. ContractPlan enumの保守プラン区分

現在、保守プランには「ライト（¥50,000/月）」と「プロ（¥80,000/月）」の2段階がありますが、DB定義上の整合が取れていません。

| enum | 値 | 問題点 |
|------|---|--------|
| `ToolStatus`（セクション4） | `MAINTENANCE_LITE` / `MAINTENANCE_PRO` | 2段階を区別 |
| `ContractPlan`（DB定義） | `MAINTENANCE` のみ | ライト/プロの区別なし |

**決定が必要な選択肢:**
- **(A)** ContractPlan enumにLITE/PROを追加し、ToolStatusは廃止して統合
- **(B)** ContractPlanはそのまま、Clientモデルに `maintenanceTier` カラムを追加して区別

### P1: Phase 2開始前に決定必須

#### P1-1. バッチジョブの実行基盤

BullMQ (Redis-backed) をジョブキューとして使用する設計だが、ワーカープロセスの実行環境が未定義。

| 選択肢 | メリット | デメリット |
|--------|---------|----------|
| **(A) Cloud Run（min instance=1で常駐）** | インフラ統一、デプロイ簡単 | 常時課金（月$30-50程度）、Cloud Runのタイムアウト制限 |
| **(B) Cloud Run Jobs** | ジョブ単位のスケーリング、コスト最適 | BullMQとの統合が複雑、Cloud Schedulerとの連携が必要 |
| **(C) Compute Engine（小型VM常駐）** | 自由度が高い | インフラ管理コスト増 |

**推奨:** (A) がシンプル。Cloud Schedulerで定刻にHTTPリクエストを送信 → Cloud RunのAPIがBullMQにジョブを投入 → 同一インスタンス内のワーカーが処理。

#### P1-2. バッチジョブの部分失敗ハンドリング

以下のケースに対する挙動を定義する必要がある:

| ケース | 現状の仕様 | 決定が必要な事項 |
|--------|----------|----------------|
| 120社中50社目でAPIレートリミット | 「次バッチに延期」 | 残り70社は翌日に持ち越し？即時リトライ待ち？優先度はどうなる？ |
| 1社の同期が途中失敗（GSCは成功、GA4が失敗） | SyncLog `partial` | 次回実行時に差分取得？全件再取得？ partial状態のデータは表示に使うか？ |
| リトライ3回全失敗 | 記載なし | 管理者通知のみ？手動再実行ボタン？自動的に翌日キューに入れる？ |
| バッチジョブ間の依存（rewrite-scoreがsync-gsc完了前に実行） | 時刻ベース | イベントドリブン（前ジョブ完了をトリガー）に変更すべきか？ |

**冪等性の設計:**
全バッチジョブはべき等（何度実行しても同じ結果）であること。`@@unique([keywordId, date])` 等のユニーク制約でUPSERTを使用する。

#### P1-3. GA4 APIクォータの検証

GA4 Data APIの制約:
- プロパティあたり: 同時10リクエスト、**日10,000トークン**
- トークン消費量はクエリの複雑さに依存（ディメンション数、日付範囲等）

**検証が必要:**
- 1社あたり5レポート（R1-R5）のbatchRunReportsで消費するトークン数の実測
- 120社で日次トークン上限（10,000 × 120 = 1,200,000トークン？ or プロジェクト全体で共有？）
- GCPプロジェクトレベルのグローバルクォータの確認

**推奨:** Phase 2開始前にパイロット5社でトークン消費量を実測すること。

#### P1-4. Pusherの同時接続数

| ユーザー種別 | 最大人数 | 同時接続見込み |
|------------|---------|-------------|
| 社内スタッフ | 25-30名 | 20名（業務時間中ほぼ常時） |
| クライアント担当者 | 120-360名 | 30-50名（ポータルを開いている間） |
| ライター | 50名+ | 10名 |
| **合計** | | **60-80名（通常）、最大150名（ピーク）** |

Startup plan（200同時接続）でピーク時はカバー可能だが、クライアントポータルの普及率次第で超過リスクあり。

**決定が必要な事項:** 接続数の監視アラート設定、超過時のプランアップグレード判断基準

#### P1-5. ダッシュボードのキャッシュ・事前集計戦略

全案件ダッシュボード（`/dashboard`）は120+社の複合指標を表示する。リアルタイムに計算すると3秒以内の表示は困難。

**推奨設計:**

| 指標 | 更新タイミング | 格納先 |
|------|-------------|--------|
| ヘルススコア | `check-alerts`バッチ（06:00）で全社一括計算 | Redis or PostgreSQL集計テーブル |
| オーガニックセッション/CV | `sync-ga4`バッチ完了時に更新 | TrafficSnapshot（既存） |
| 未読チャット数 | メッセージ送信時にインクリメント、既読更新時にリセット | Redis |
| 要対応フラグ | `check-alerts`バッチで更新 | Redis or PostgreSQL |

フロントエンドは事前集計済みデータを取得するだけで、重い集計クエリを実行しない設計とする。

### P2: Phase 3開始前に決定必須

#### P2-1. Looker Studioレポート埋め込みの設計

Looker StudioをBigQueryに接続し、ダッシュボード内にiframeで埋め込む方式を採用。

**レポートセクション構成（1セクション = Looker embed + 所感テキスト）:**

| 要素 | 実装 |
|------|------|
| グラフ表示 | Looker Studio埋め込み（`/embed/reporting/...` URL） |
| AI所感 | Claude APIで生成したドラフトをテキストエリアに表示。コンサルタントが編集可能 |
| PDF出力 | `@media print` CSSでサイドバー・ヘッダー非表示 + `window.print()` |

**Looker Studioテンプレート管理:**
- レポート種別ごとにテンプレートを1つ作成
- クライアントごとにBQデータセットをパラメータで切り替え
- 埋め込みURLにパラメータを付与してクライアント別表示を実現

**決定が必要な事項:**
- Looker Studioの共有設定（組織内共有 or リンク共有）
- iframeのcross-origin制約への対応（同一Google Workspace内なら問題なし）
- Looker Studioテンプレートの作成・管理フロー

#### P2-2. AI基盤の選定（検討中）

所感生成・改善提案に使用するAIモデルは以下の候補があり、Phase 3開始前に確定する。

**候補モデル比較（月2,040回の所感生成 + 月240回の改善提案を想定）:**

| モデル | 月額目安 | GCPネイティブ | 日本語品質 | 備考 |
|--------|---------|:---:|--------|------|
| **Gemini 2.5 Flash** | **約¥660** | ✅ Vertex AI | 良好 | GCP課金統合。定型所感に十分な品質 |
| **Gemini 2.5 Pro** | 約¥2,680 | ✅ Vertex AI | 高品質 | 深い分析にも対応。Flashとの使い分けも可 |
| **Claude Sonnet 4.6** | 約¥5,050 | ❌ 外部API | 最高品質 | 日本語の自然さは最高だがコスト・外部依存 |
| **Claude Haiku 4.5** | 約¥1,680 | ❌ 外部API | 良好 | Sonnetより安価だが外部依存は同じ |

**検討ポイント:**
- **GCPネイティブの利点**: Vertex AI経由なら外部APIキー管理が不要。認証・課金が既存GCPプロジェクトに統合される
- **ハイブリッド案**: 月次所感（定型・大量）はGemini Flash、月2案の改善提案（高度な推論）はClaude Sonnetという使い分けも有効
- **コスト差は小さい**: 最安¥660〜最高¥5,050/月の範囲。品質とインフラ統合を優先して選定すべき

**プロンプト設計（要プロトタイプ）:**

```
入力コンテキスト（1社あたり）:
- 当月/前月/前年のKPI数値（セッション, CV, ユーザー数）
- KWフォーカスデータ（TOP変動5件）
- コンテンツ進捗状況
- 前月レポートの所感（トーンの一貫性のため）

出力:
- セクションごとの所感テキスト（200-400文字/セクション）
- JSON形式で構造化
```

**失敗時のフォールバック:**
- AI API障害時: 所感欄を「AI所感の生成に失敗しました。手動で入力してください。」として空欄で先に進む
- 不適切な出力時: テンプレートベースのフォールバック所感を表示（「前月比でオーガニックセッションが{diff}%{増加/減少}しました。」等）

#### P2-3. ナレッジベースの類似検索方式

| 選択肢 | 実装コスト | 精度 | 備考 |
|--------|----------|------|------|
| **(A) PostgreSQL全文検索（pg_trgm）** | 低 | 中 | 日本語の形態素解析が弱い |
| **(B) pgvector + Embedding** | 中 | 高 | Claude/OpenAI Embeddingで意味検索が可能 |
| **(C) 外部検索サービス（Algolia等）** | 高 | 高 | 追加コスト・依存 |

**推奨:** Phase 3時点では **(A)** でMVP。レコード数が増加し検索精度が課題になった時点で **(B)** に移行。

### P3: 全Phase横断の技術課題

#### P3-1. マルチテナント分離の実装パターン

Prismaミドルウェアの制限:
- `$queryRaw` / `$executeRaw` には適用されない
- BigQueryクエリには一切効かない
- `User` テーブル等、`projectId` を持たないテーブルには適用不可

**推奨設計:**

| レイヤー | 分離方式 |
|---------|---------|
| PostgreSQL（Prisma経由） | Prismaミドルウェア + `@@index([projectId])` |
| PostgreSQL（生SQL） | 使用禁止。全クエリをPrisma経由に限定 |
| BigQuery | クエリに必ず `WHERE project_id = @projectId` を含める。共通ユーティリティ関数で強制 |
| API層 | 全エンドポイントで `requireProjectAccess(userId, projectId)` ミドルウェアを適用 |

**テスト戦略:**
- E2Eテストで「ユーザーAがプロジェクトBのデータにアクセスできないこと」を全主要エンドポイントで検証
- テストシードに2テナント分のデータを投入し、クロステナントアクセスの防止を網羅的にテスト

#### P3-2. ログ・監視基盤

| 要素 | 推奨 | 用途 |
|------|------|------|
| **構造化ログ** | Cloud Logging（Cloud Run標準出力で自動連携） | API応答時間、バッチジョブの進捗・エラー |
| **エラートラッキング** | Sentry（既定） | 未ハンドルの例外、フロントエンドエラー |
| **死活監視** | UptimeRobot（既定） | エンドポイントの応答確認 |
| **メトリクス** | Cloud Monitoring | CPU/メモリ使用率、DB接続数、キューの滞留数 |
| **ログ保持期間** | 30日（Cloud Logging標準） | 要件次第で延長 |
...
|------|------|
| **ツール** | Prisma Migrate |
| **本番適用** | GitHub Actionsのデプロイパイプラインに組み込み。`prisma migrate deploy` を自動実行 |
| **ロールバック** | Prisma Migrateはロールバック非対応。破壊的変更は「新カラム追加 → データ移行 → 旧カラム削除」の3ステップで段階的に実施 |
| **ダウンタイム** | 原則ゼロダウンタイム。テーブルロックが発生するALTER TABLE（大テーブルへのカラム追加等）は深夜メンテナンスウィンドウで実施 |

#### P3-4. デプロイ戦略

| 項目 | 方針 |
|------|------|
| **デプロイ方式** | Cloud Runのローリングデプロイ（デフォルト） |
| **ロールバック** | Cloud Runのリビジョン切り戻し（`gcloud run services update-traffic`） |
| **バッチジョブ** | アプリケーションと同一コンテナ内。Cloud Schedulerからの定刻HTTPリクエストでトリガー |
| **環境** | staging（ステージング）+ production（本番）の2環境。mainブランチ → staging自動デプロイ → 手動承認で本番デプロイ |
| **フィーチャーフラグ** | Phase間の段階リリースに使用。環境変数 or DBベースのフラグ管理 |

#### P3-5. フロントエンド技術選定の補足

| 要素 | 推奨 | 理由 |
|------|------|------|
| **データフェッチ** | TanStack Query (React Query) | キャッシュ管理、楽観的更新、リアルタイムデータとの統合が容易 |
| **クライアント状態** | Zustand（軽量）or Context API | カンバンD&D状態、フィルタ条件等のUI状態管理 |
| **リアルタイム統合** | Pusherイベント受信 → TanStack Queryのキャッシュを手動更新 | APIキャッシュとリアルタイムデータの一貫性を保つ |
| **テーブル** | TanStack Table | 120+社のソート/フィルタ/仮想スクロール対応 |

**RSC（React Server Components）とクライアントコンポーネントの境界:**
- サーバーコンポーネント: ページレイアウト、初期データフェッチ、SEOメタ
- クライアントコンポーネント: チャート、カンバン、チャット、フィルタ付きテーブル、リアルタイム更新が必要なUI

#### P3-6. 非同期ジョブのステータス管理（レポート生成等）

レポート生成（`POST /api/clients/[id]/reports/generate`）等の非同期処理に対するステータス確認APIが必要。

**追加エンドポイント:**

| メソッド | エンドポイント | 説明 |
|---------|-------------|------|
| `GET` | `/api/jobs/[jobId]` | ジョブの進捗確認（`queued` / `processing` / `completed` / `failed`） |

**完了通知:** Pusherで `job:completed` イベントを送信。フロントエンドはPolling不要。
...
| **PPTX** | - | PowerPointファイルの形式。現行レポートで使用中 |
| **Looker Studio** | 旧Googleデータポータル | Googleのダッシュボードツール。BigQueryに接続し、レポートセクションをiframeで埋め込み表示。無料（Googleアカウントがあれば利用可能） |
