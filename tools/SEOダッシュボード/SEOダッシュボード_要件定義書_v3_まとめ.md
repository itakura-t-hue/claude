...
- **年間効果（解約防止+保守転換+工数削減）**: ¥30,360,000
- **初年度ROI**: (30,360,000 - 2,230,000 - 11,200,000) / 11,200,000 = **151%**
- **投資回収期間: 4-5ヶ月**
- 2年目以降は年間¥28,000,000以上の純効果
...
| **データ取得** | GSC/GA4 APIからのデータ同期 | 共有用Gmailアカウント（クライアントから権限付与済み） | `webmasters.readonly`, `analytics.readonly` |
...
**補足:** 現行運用は「共有Gmail 1アカウント」でGSC/GA4に閲覧者権限を付与してもらう方式（セクション1.6参照）。この場合は **(A)** が整合するが、リフレッシュトークン失効時の再認証フロー（管理者が手動で共有Gmailで再ログイン）を設計に含める必要がある。
