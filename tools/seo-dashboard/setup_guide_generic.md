...
本ガイドは **パターンA（OAuth + ローカル実行）** を前提に STEP 1〜7 を記載する。
中長期的な改善として **パターンB / C** への移行パスを付録に記載。現状の運用課題や無料枠圧迫の試算は末尾の「現状の運用と課題」を参照。
...
自社では約6名のSEOディレクターが、それぞれ自身のローカルPC上でPythonスクリプトを実行してダッシュボードのデータ更新を行っている。さらに1つのGoogleアカウント（作業用アカウント）に約20サイト分のGA4/GSC/BQが紐付いている状態で、属人化・再現性の低さが強く、運用としては雑すぎる。
...
| アカウント集中 | 1アカウントに20サイト紐付け → そのアカウントのクォータ・セキュリティ事故が全案件に波及 |
...
| 対象 | 現状（20サイト） | 100サイト想定 |
|------|----------------|-------------|
| GSC API | 余裕 | 余裕 |
| GA4 Data API | 余裕 | 余裕 |
| BQストレージ | 1年目で超過（月数十円〜） | 2〜3ヶ月で超過（月数百円〜） |
| BQストリーミング挿入 | サンドボックス挙動要検証 | 要検証 |
| Sheets API | 瞬間的制限ヒットの可能性 | 高確率で制限ヒット |
| OAuthトークン | 余裕 | 枯渇 |

**結論**: 無料枠自体は当面問題にならないが、**実質のボトルネックは運用構造（PC依存・トークン失効・属人化）**。金額より先に人的コストで破綻する。
...
1. **短期（〜1ヶ月）**: 現状維持。週次でトークン再認証を運用ルール化
2. **中期（1〜3ヶ月）**: **パターンB に移行**（トークン失効解消。スクリプト改修のみで済み、会社承認不要）
3. **長期（3〜6ヶ月）**: 会社判断が取れ次第 **パターンC に移行**（PC依存・属人化・監視不在を一挙に解消）
