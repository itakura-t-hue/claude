...
> **このドキュメントの位置付け**
> 本ガイドは **「SEO統合ダッシュボード 要件定義書 v3.0」の開発が完成するまでの現行運用（Phase 0）** を対象とした、データパイプライン構築手順書です。
> GA4 → BigQuery → Google Spreadsheet への自動反映を各案件に個別に設定する手順を記載します。
> v3ダッシュボードが稼働すれば本ガイドは不要になりますが、現在進行中の案件や新規案件のオンボーディングに使用します。
> v3ダッシュボードの本体仕様は `SEOダッシュボード_要件定義書_v3_まとめ.md` および `SEOダッシュボード_要件定義書_v3_詳細版.html` を参照してください。
